...
# 4. Points both semantic models to this Microsoft-hosted data source.
# 5. Runs a populated-data DAX validation against both semantic models.
#
...
# GitHub](https://github.com/microsoft/fabric-data-agent-workshop/tree/main/documentation/lab-instructions).
...
EXPECTED_FOLDER_NAME = "__LOGICAL_ID__"
...
# ## Step 5 - Point both semantic models to the Microsoft data source
#
# The imported PBIX files contain populated cached data. This step updates their
# `DataFolder` parameter so future refreshes use the Microsoft-hosted release
# instead of the repository where the PBIX files were originally authored.

# CELL ********************

model_data_source = f"{RAW_BASE_URL}/data/mfg-ops-data"
semantic_model_artifacts = [
    artifact
    for artifact in artifacts_by_id.values()
    if artifact["type"] == "SemanticModel"
]
if len(semantic_model_artifacts) != len(PBIX_ASSETS):
    raise RuntimeError(
        "Expected one imported semantic model per PBIX asset, but received "
        f"{len(semantic_model_artifacts)} semantic models."
    )

for artifact in semantic_model_artifacts:
    update_parameters_url = (
        "https://api.powerbi.com/v1.0/myorg/groups/"
        f"{workspace_id}/datasets/{artifact['id']}/Default.UpdateParameters"
    )
    ...
    if update_response.status_code != 200:
        raise RuntimeError(
            f"Could not update the DataFolder parameter for "
            f"{artifact['name'] or artifact['id']}: "
            f"HTTP {update_response.status_code} {update_response.text}"
        )
    print(
        "Updated semantic model source:",
...
# ## Step 6 - Verify populated data in both semantic models
#
